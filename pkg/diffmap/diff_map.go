...
var (
	...
	diff_BB_coverage_threshold         = 1000
	diff_func_coverage_ratio     int64 = 2
	diff_func_coverage_threshold       = 10000
)
...
func (dm *DiffMap) CountDiffPrio(cover []uint32) (prio int64) {
	for _, pc := range cover {
		if dm.BB_map.Test(pc) { // Test BB Coverage
			(*dm.BB_hitcount)[pc]++
			// priority algorithm params can be adjusted
			if (*dm.BB_hitcount)[pc] < diff_BB_coverage_threshold {
				prio += diff_BB_coverage_ratio
			}
		}
		if dm.FuncDiffMatch(pc, true) { // Test Function Coverage
...
func (dm *DiffMap) FuncDiffMatch(addr uint32, count_hit bool) bool {
...
func (dm *DiffMap) FilterCover(cover []uint32) (filtered []uint32) {
	for _, pc := range cover {
		if dm.FuncDiffMatch(pc, false) {
