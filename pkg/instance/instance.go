...
func FuzzerCmd(args *FuzzerCmdArgs) string {
	...
	return fmt.Sprintf("%v -executor=%v -name=%v -arch=%v%v -manager=%v -sandbox=%v"+
		" -procs=%v -cover=%v -cover_blind=%v -cover_ipt=%v -cover_diff=%v -diff_cover_only=%v -bokasan=%v -debug=%v -test=%v%v%v%v",
		args.Fuzzer, args.Executor, args.Name, args.Arch, osArg, args.FwdAddr, args.Sandbox,
		args.Procs, args.Cover, args.CoverBlind, args.CoverIpt, args.CoverDiff, args.DiffCoverOnly, args.BoKASAN, args.Debug, args.Test, runtestArg, verbosityArg, optionalArg)
}
